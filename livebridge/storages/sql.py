    ...
    async def setup(self):
        ...
        try:
            ...
            if not await engine.has_table(self.table_name):
                ...
                await conn.execute("CREATE INDEX `lb_last_updated` ON `{}` (`source_id` DESC,`updated` DESC);".format(self.table_name))
                await conn.execute("CREATE INDEX `lb_post` ON `{}` (`target_id` DESC,`post_id` DESC);".format(self.table_name))
                await conn.close()
                return True
        except Exception as e:
            logger.error("[DB] Error when setting up SQL table: {}".format(e))
        return False
